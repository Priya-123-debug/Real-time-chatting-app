...
function Chatcontainer({ selectedUser, onBack }) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const res = await sendMessage(selectedUser._id, formData);
      ...
      <div className="shrink-0 flex items-center justify-between px-4 md:px-6 py-3 md:py-4 border-b border-white/5 bg-[#141924]">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="relative shrink-0">
            ...
            <span
              className={`absolute bottom-0 right-0 w-3 h-3 rounded-full border-2 border-[#141924] ${
                isOnline ? "bg-emerald-400" : "bg-gray-500"
              }`}
            />
          </div>
          <div className="min-w-0">
            <p className="text-white font-semibold font-['Sora'] truncate">
              {selectedUser.username}
            </p>
            <span
              className={
                isOnline ? "text-emerald-400 text-xs" : "text-gray-500 text-xs"
              }
            >
      ...
      <div className="flex-1 min-h-0 overflow-y-auto px-4 md:px-5 py-4 space-y-3">
        {loading ? (
          ...
          messages.map((message) => {
            ...
            const isVideo = hasMedia && message.media.type.startsWith("video/");

            const isAudio = hasMedia && message.media.type.startsWith("audio/");
              ...
              >
                <div className="relative max-w-[78%] md:max-w-[65%]">
                  <div
                    onClick={() =>
                      !message.deleted &&
                      setActiveMessageId(isMenuOpen ? null : message._id)
                    }
                  ...
                  >
                    {message.deleted ? (
                      <p className="text-sm flex items-center gap-2">
                        <FaUserSlash className="text-xs" /> This message was
                        deleted
                      </p>
                    ) : (
                      <>
